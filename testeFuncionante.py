...
displayImage = sg.Image(
    fileAbsDir + '/' + fileList[0] + ".png", key="imageContainer")
...
while True:
    ...
    if event == "Previous" and index > 0:
        if newFilename != "":
            os.rename(os.path.join(
                fileAbsDir, fileList[index] + ".png"), os.path.join(fileAbsDir, newFilename + ".png"))
    ...
    if index >= len(fileList):
        break
    inputWindow.FindElement("imageContainer").Update(
        fileAbsDir + '/' + fileList[index] + ".png")
    inputWindow.TKroot.title(
        "[{}/{}] Please input - {}".format(index + 1, len(fileList), fileList[index]))

inputWindow.Close()

sg.Window("Finished!", [[sg.Text(
    "Finished updating {} image labels.".format(len(fileList)))], [sg.OK()]]).Read()
